...
export async function getPokemonDataRaw(page?: number): Promise<{ results: PokemonData[], next: boolean }> {
    const limit = 20;    
    const skip = (limit * (page|| 0)) - 20;

    const url = page ?  `https://pokeapi.co/api/v2/pokemon?limit=${limit}&offset=${skip}` : 
...
export async function getAllPokemon(page?: number): Promise<{ results: Pokemon[], next: boolean }> {
    const time = Date.now();
